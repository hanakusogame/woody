...
export class Config extends g.FilledRect {
	...
	constructor(scene: g.Scene, x: number = 0, y: number = 0) {
		...
		const colors = ["#303030", "gray", "black", "white", "green", "navy"];
		let colorNum = 0;
		//背景色
		base.append(new g.Label({
			scene: scene,
		...
		const sprColor = new g.FilledRect({ scene: scene, x: 132, y: 152, width: 106, height: 36, cssColor: colors[colorNum], touchable:true });
